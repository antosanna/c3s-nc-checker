...
class Checker_test():
    ...
    def create_netcdffile(self, cdldir):
        ...
        for f in listcdl:
            ...
            try:
                ...
                os.popen(command)
                fct.prBlack(
                    "Creation of temporary NetCDF file: " +
                    ncfile +
                    " from " +
                    f)
    ...
    def test_compliantfiles(self):
        ...
        listsubdir = fct.get_immediate_subdirectories(os.path.join(
            os.path.dirname(os.path.realpath(__file__))) + cdldirmain)
        ...
        for subdir in listsubdir:
            ...
            for ncfile in listnc:
                ...
                try:
                    ...
                    self.failed += 1
                    fct.prRed(
                        " FAILED - Checker status should be 1 (Successful)")
    ...
    def test_noncompliantfiles(self):
        ...
        for subdir in listsubdir:
            ...
            for ncfile in listnc:
                ...
                status = C3S_checker.check(['-t', subdir, ncfile])
                print "Check " + ncfile + " with configuration [" + subdir.upper() + "]"
                try:
                    assert status == 0
                    ...
                    self.failed += 1
                    fct.prRed(
                        " FAILED - Checker status should be 0 (Unsuccessful)")
...
if __name__ == "__main__":
    ...
    fct.prCyan("Plateform: " + str(platform.system()) +
    ...
    if failed > 0:
        ...
        fct.prRed("========== Test session end - Elapsed time : " +
                  str(e[0]) + " minutes " + str(e[1]) + "seconds ===========")

    else:
        fct.prGreenBold("Tests OK")
        fct.prGreen("========== Test session end - Elapsed time : " +
                    str(e[0]) + " minutes " + str(e[1]) + "seconds ===========")

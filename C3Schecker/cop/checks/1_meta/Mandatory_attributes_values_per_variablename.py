                ...
                for attr in mandatoryattributes :
                    ...
                    if (attr in cfattrs) and (str(attr) in regex_special):
                        if  not re.match(mandatoryattributes[attr],v.getncattr(attr)):
                            ...
                            self.status = 0
                            self.logger.error("[%s]-Wrong [%s] value - Variable [%s]; [%s] does not match [%s]", str(self.getcheckname(self.addinfo)), str(attr), str(k), str(mandatoryattributes[attr]), str(v.getncattr(attr)))

					...
					for l,m in mandatoryintervals.iteritems():
						...
						try:
							...
							if len(valuesintervals) > 0:
								self.status = 0
								self.logger.error("[%s]- [%s] intervals must be %s  - Some other intervals has been found - First: %s  ", str(self.ref) , str(l), str(m), str(  str(valuesintervals[0][0:2]) + "=interval " + str(valuesintervals[0][2]) ) )

        ...
        for k, v in self.cfcollection:
            ...
            if fm is not None and (fv is None and fma is None):
                self.check_msgs_logger.error(
                    "[%s]- Flag_meanings attribute exists and no flag_values or flag_masks for variable [%s]",
                    str(ref),
                    str(k),
                )
                self.status = 0
            ...
            if fm is None and (fv is not None or fma is not None):
                self.check_msgs_logger.error(
                    "[%s]- Flag_values or flag_masks exists and no flag_meanings for variable [%s]",
                    str(ref),
                    str(k),
                )
                self.status = 0
            ...
            if v.standard_name:
                ...
                if (
                    ...
                    in [
                        i
                        for i in list(self.cfref.cf_standard_names_modifiers().keys())
                        if "flag" in i
                    ]
                    and fm is None
                ):
                    self.check_msgs_logger.error(
                        "S[%s]- tandard Name modified to a flag but flag_meanings does not exist for variable [%s]",
                        str(ref),
                        str(k),
                    )
                    self.status = 0
            ...
            if not (isinstance(fm, str)):
                self.status = 0
                self.check_msgs_logger.error(
                    "[%s]- Flag_meanings attribute [%s] must be a string for variable [%s]",
                    str(ref),
                    str(type(fm)),
                    str(k),
                )
            ...
            if len([i for i in fm_items if rname.match(str(i))]) != len(fm_items):
                self.status = 0
                self.check_msgs_logger.error(
                    "[%s]- Flag_meanings syntax incorrect for variable [%s]",
                    str(ref),
                    str(k),
                )
                continue
            ...
            if fv is not None:
                ...
                if isinstance(fv, np.ndarray):
                    ...
                    fv, str
                ):  # not sure fully necessary- considering blank as separator
                    fv = list(map(str.strip, fv.split(" ")))
                else:
                    self.status = 0
                    self.check_msgs_logger.error(
                        "[%s]- Flag_values must be a list for variable [%s]",
                        str(ref),
                        str(k),
                    )
                    continue
            ...
            for i in fv:
                if not np.issubdtype(type(i), v.dtype.type):
                    ...
                    isok = False
                    self.check_msgs_logger.error(
                        "[%s]- Flag_values values must have the same type as variable [%s] [%s]",
                        str(ref),
                        str(k),
                        str(v.dtype),
                    )
            ...
            if len(fv) != len(fm_items):
                self.status = 0
                self.check_msgs_logger.error(
                    "[%s]- Flag_values must have the same number of values as flag_meanings for variable [%s]",
                    str(ref),
                    str(k),
                )
                continue
            ...
            if len(fv) != len(set(fv)):
                self.status = 0
                self.check_msgs_logger.error(
                    "[%s]- Flag_values values must be unique for variable [%s]",
                    str(ref),
                    str(k),
                )
                continue
            ...
            if fma is not None:
                ...
                if fma.dtype != v.dtype:
                    self.status = 0
                    self.check_msgs_logger.error(
                        "[%s]- Flag_masks values must have the same type as variable [%s] [%s]",
                        str(ref),
                        str(k),
                        str(v.dtype),
                    )
                ...
                if v.dtype not in [
                    ...
                    self.check_msgs_logger.error(
                        "[%s]- Variable [%s] type not appropriate for flag_masks:  [%s]",
                        str(ref),
                        str(k),
                        str(v.dtype),
                    )
                ...
                if len([i for i in fma if i == 0]) != 0:
                    self.status = 0
                    self.check_msgs_logger.error(
                        "[%s]- Flag_values values must non-zero for variable [%s]",
                        str(ref),
                        str(k),
                    )
                    continue
            ...
            if fma is not None and fv is not None:
                ...
                if not bitwiseAND:
                    self.status = 0
                    self.check_msgs_logger.error(
                        "[%s]- Bitwise AND of flag_values and flag_masks not equal to flag_value for variable [%s]",
                        str(ref),
                        str(k),
                    )
                    continue
